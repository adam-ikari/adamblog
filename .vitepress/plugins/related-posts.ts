  ...
  return {
    ...
    buildStart() {
      ...
      for (const file of files) {
        ...
        const article: Article = { path, title, terms: tf, norm: 0 }
      ...
      // 每篇取 top-N 相似
      const related: Record<string, { title: string; link: string }[]> = {}
      for (const a of articles) {
        const scored = articles
          ...
          .slice(0, TOP_N)
        related[a.path] = scored.map((s) => ({ title: s.b.title, link: s.b.path }))
      }
