...
// node_modules/vitepress-plugin-blog/dist/composables/useBlogSidebar.js
var a2 = ref([]);
var u = ref(false);
var p2 = false;
function h2() {
  if (typeof document > "u") return [];
  const t2 = document.getElementById("vitepress-blog-sidebar");
  if (!(t2 != null && t2.textContent)) return [];
  try {
    const e2 = JSON.parse(t2.textContent);
...
function v(t2) {
  ...
  if (typeof document > "u") return;
  const e2 = t2.find(
    (r) => r.text.toLowerCase().includes("recent")
  );
  if (!(e2 != null && e2.items)) {
    console.log("[vitepress-plugin-blog] No recent posts section found in sidebar data");
    return;
  }
  const o3 = document.querySelectorAll(".VPSidebarItem.level-0");
  for (const r of o3) {
    const s3 = r.querySelector(":scope > .item .text");
    if ((((d2 = s3 == null ? void 0 : s3.textContent) == null ? void 0 : d2.trim().toLowerCase()) || "").includes("recent")) {
      console.log("[vitepress-plugin-blog] Found Recent posts section, updating...");
      const b = r.querySelectorAll(":scope > .items > .VPSidebarItem");
      e2.items.forEach((n, y) => {
        const l = b[y];
        if (l) {
          const i2 = l.querySelector(".text");
          i2 && i2.textContent !== n.text && (console.log(`[vitepress-plugin-blog] Updating: "${i2.textContent}" -> "${n.text}"`), i2.textContent = n.text);
          const c2 = l.querySelector("a");
          if (c2 && n.link) {
            const f = c2.getAttribute("href") || "";
            !f.endsWith(n.link) && !f.endsWith(n.link + ".html") && c2.setAttribute("href", n.link);
          }
        }
...
function S() {
  if (!p2) {
    p2 = true;
    try {
      const e2 = import.meta.hot;
      e2 && typeof e2.on == "function" && (e2.on("blog-sidebar-update", async (o3) => {
        console.log(
          "[vitepress-plugin-blog] Sidebar HMR update received:",
          Array.isArray(o3) ? o3.length : 0,
          "sections"
        ), Array.isArray(o3) && (a2.value = [...o3], await nextTick(), setTimeout(() => {
          v(o3);
        }, 50));
...
function C() {
  return onMounted(() => {
    if (!u.value) {
      const t2 = h2();
      t2.length > 0 && (a2.value = t2, u.value = true);
    }
    S();
  }), {
    sidebar: a2,
...
// node_modules/vitepress-plugin-blog/dist/sidebar.js
...
function D(u2, y = {}) {
  const {
    basePath: x2 = "/blog/",
    recentPostsCount: S2 = 5,
    allPostsLabel: w = "All posts",
    recentPostsLabel: F2 = "Recent posts",
    recentPostsCollapsed: k = false
  } = y, g = u2.slice(0, S2), h3 = [
    {
      text: "Blog",
      items: [
        { text: w, link: x2 }
...
function A(u2, y = {}) {
  const {
    ...
    recentPostsLabel: F2 = "Recent posts",
    recentPostsCollapsed: k = false,
    postsPattern: g = "blog/posts/**/*.md"
  } = y;
  function h3(e2, l) {
    const t2 = [], n = e2.split("/");
    function i2(c2, s3) {
      if (s3 >= n.length) return;
      const a3 = n[s3], b = s3 === n.length - 1;
      if (import_fs.default.existsSync(c2))
        if (a3 === "**") {
          const r = import_fs.default.readdirSync(c2, { withFileTypes: true });
          for (const f of r) {
            const o3 = import_path.default.join(c2, f.name);
            if (f.isDirectory())
              i2(o3, s3), i2(o3, s3 + 1);
            else if (b || s3 === n.length - 2) {
              const P2 = n[s3 + 1] || a3;
              new RegExp("^" + P2.replace(/\*/g, ".*").replace(/\?/g, ".") + "$").test(f.name) && t2.push(o3);
            }
          }
        } else if (a3.includes("*")) {
          const r = new RegExp("^" + a3.replace(/\*/g, ".*").replace(/\?/g, ".") + "$"), f = import_fs.default.readdirSync(c2, { withFileTypes: true });
          for (const o3 of f)
            if (r.test(o3.name)) {
              const P2 = import_path.default.join(c2, o3.name);
              o3.isDirectory() && !b ? i2(P2, s3 + 1) : o3.isFile() && b && t2.push(P2);
            }
        } else {
          const r = import_path.default.join(c2, a3);
          import_fs.default.existsSync(r) && (b ? t2.push(r) : i2(r, s3 + 1));
        }
    }
    return i2(l, 0), t2;
  }
  ...
  for (const e2 of m2)
    try {
      const l = import_fs.default.readFileSync(e2, "utf-8"), { data: t2 } = (0, import_gray_matter.default)(l);
      if (t2.published === false) continue;
      const i2 = "/" + import_path.default.relative(u2, e2).replace(/\\/g, "/").replace(/\.md$/, "").replace(/\/index$/, "/");
      C2.push({
        title: t2.title || "Untitled",
        date: t2.date || "",
        url: i2,
        published: t2.published !== false
      });
    } catch (l) {
      console.warn(`[vitepress-plugin-blog] Could not parse ${e2}:`, l);
    }
  C2.sort((e2, l) => {
    const t2 = e2.date ? Date.parse(e2.date) : 0;
    return (l.date ? Date.parse(l.date) : 0) - t2;
  });
  const L = C2.slice(0, S2), T2 = [
    {
      text: "Blog",
      items: [
        { text: w, link: x2 }
...
// node_modules/vitepress-plugin-blog/dist/index.js
var P = null;
var c = null;
async function q() {
  const [
    ...
    import("./BlogPostLayout.vue-KYMF7G7S.js"),
    import("./BlogLayoutWrapper.vue-43D7G3CB.js"),
    import("./BlogIndex.vue-SVSEO4NZ.js"),
    import("./BlogCard.vue-MMGOGPJN.js"),
    import("./BlogFilters.vue-IQYCIA6D.js"),
    import("./BlogPagination.vue-T3QKXGQE.js")
  ]);
  o3.default, P = e2.default, c = t2.default, a3.default, s3.default, r.default;
}
var m = null;
async function M() {
  if (m) return m;
  try {
    return m = await import("vitepress"), m;
  } catch {
    return null;
  }
}
...
async function I() {
  if (!B) {
    B = true;
    try {
      const { blogPosts: o3 } = await import("virtual:blog-posts");
      Array.isArray(o3) && (p4.value = o3);
    } catch {
...
function K(o3) {
  ...
  if (typeof document > "u") return;
  const e2 = o3.find(
    (s3) => s3.text.toLowerCase().includes("recent")
  );
  if (!(e2 != null && e2.items))
    return;
  const t2 = document.querySelectorAll(".VPSidebarItem.level-0");
  for (const s3 of t2) {
    const r = s3.querySelector(":scope > .item .text");
    if ((((a3 = r == null ? void 0 : r.textContent) == null ? void 0 : a3.trim().toLowerCase()) || "").includes("recent")) {
      const y = s3.querySelectorAll(":scope > .items > .VPSidebarItem");
      e2.items.forEach((n, i2) => {
        const l = y[i2];
        if (l) {
          const u2 = l.querySelector(".text");
          u2 && u2.textContent !== n.text && (console.log(`[vitepress-plugin-blog] Sidebar: "${u2.textContent}" -> "${n.text}"`), u2.textContent = n.text);
          const b = l.querySelector("a");
          if (b && n.link) {
            const h3 = b.getAttribute("href") || "";
            !h3.endsWith(n.link) && !h3.endsWith(n.link + ".html") && b.setAttribute("href", n.link);
          }
        }
      });
      break;
...
function T() {
  if (!x) {
    ...
    try {
      const e2 = import.meta.hot;
      e2 && typeof e2.on == "function" && (e2.on("blog-posts-update", (t2) => {
        console.log("[vitepress-plugin-blog] HMR update received:", Array.isArray(t2) ? t2.length : 0, "posts"), Array.isArray(t2) && (p4.value = [...t2]);
      }), e2.on("blog-sidebar-update", async (t2) => {
        console.log("[vitepress-plugin-blog] Sidebar HMR update received"), Array.isArray(t2) && (V.value = [...t2], await nextTick(), setTimeout(() => {
          K(t2);
        }, 50));
...
var F = async () => (await import("./useBlogNavigation-6RYLFY6E.js")).useBlogNavigation;
function W(o3, e2 = {}) {
  if (!o3)
  ...
  const t2 = e2.blogFrontmatterKey ?? "blogPost", a3 = o3.Layout;
  if (!a3)
    throw new Error("[vitepress-plugin-blog] baseTheme.Layout is required. Make sure you pass a valid VitePress theme.");
  e2.posts && e2.posts.length > 0 && p4.value.length === 0 && (p4.value = e2.posts);
  const s3 = shallowRef(null), r = shallowRef(null), g = shallowRef(null), y = defineComponent({
    name: "BlogAwareLayout",
    setup(n, { slots: i2 }) {
      var u2;
      const l = ((u2 = r.value) == null ? void 0 : u2.call(r)) ?? null;
      return onMounted(() => {
        T();
      }), provide(e, p4), provide(s, a3), provide(o, shallowRef(l)), provide(t, s3), () => g.value ? h(g.value, { blogFlagKey: t2 }, i2) : h(a3, null, i2);
    }
  });
  return {
    extends: o3,
    Layout: y,
    async enhanceApp(n) {
      const { app: i2 } = n, l = await M();
      l && (r.value = l.useData, s3.value = l.withBase), await q(), g.value = P, await I(), c && (i2.component("BlogIndex", c), i2.component("BlogPostList", c), i2.component("BlogHome", c));
    }
  };
}
export {
  ...
  e as blogPostsKey,
  a as formatDate,
  D as generateBlogSidebar,
  A as generateBlogSidebarFromFiles,
  i as getGitHubAvatarUrl,
  s2 as isGitHubUsername,
  o2 as parseDate,
  F as useBlogNavigation,
  p as useBlogPosts,
  C as useBlogSidebar,
  o as vitePressDataKey,
  t as withBaseKey,
  W as withBlogTheme
};
...
strip-bom-string/index.js:
   ...
   *)
*/
//# sourceMappingURL=vitepress-plugin-blog.js.map
