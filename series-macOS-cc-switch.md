---
title: macOS 配置 cc-Switch 系列
date: 2026-06-09
category: 技术探索
tags: [macOS, cc-Switch, Claude-Code, 系列]
---

# macOS 配置 cc-Switch 系列

本系列教程帮助你在 macOS 上配置 cc-Switch 命令行工具，实现 Claude Code 的多提供商/账户切换。

<SeriesCardList :articles="[
  { title: 'macOS 开发环境准备教程', desc: 'Homebrew、Node.js、终端配置', link: '/posts/macOS-环境准备教程', order: 1 },
  { title: 'macOS 配置 cc-Switch 教程', desc: '安装、配置、使用场景', link: '/posts/macOS-配置cc-Switch教程', order: 2 },
]" />

> 💡 第 1 篇同时属于 **macOS 配置 Claude Code 系列**，第 2 篇也属于该系列。
