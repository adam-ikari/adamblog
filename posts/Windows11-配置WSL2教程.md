...
在 Windows 上做开发，总绕不开一个尴尬：你想要的工具链大多长在 Linux 上，但工作环境又是 Windows。装虚拟机太重，双系统切换太烦，WSL2 就是来填这个坑的。它是微软的第二代 Linux 子系统，跑的是真正的 Linux 内核，不是模拟，兼容性比第一代强出一截。

Docker、systemd 这些依赖内核特性的东西在 WSL2 里都能跑，文件系统 I/O 也比 WSL1 快得多。Windows 和 Linux 的文件还能互相访问，不用来回倒。说穿了，你不用离开 Windows，就能拿到一个基本完整的 Linux 开发环境。
...
性能这块有个原则：Linux 项目就放在 WSL 文件系统里（比如 `~/projects`），这是最快的。反过来，从 WSL 去读 Windows 那边的文件，跨文件系统会有损耗。另外别在 Windows 下直接改 WSL 里的文件，权限容易出问题，这种坑踩一次就够。
...
装完这一圈，你在 Windows 上就有了一个能干活的 Linux 环境：装好了 WSL2、选好了发行版、配好了镜像源和资源限制，常用开发工具也齐了。常见的那几个坑——启动失败、DNS 解析、时间不同步——心里也有数了。
