...
最近开始学 Vue，打算边学边把过程中踩的坑和理解记下来，后续会陆续更新。

工程创建这一步参考了官网文档：[《快速上手 | Vue.js》](https://cn.vuejs.org/guide/quick-start.html)
...
我用的 Node.js 版本是 18，下面这条命令会基于 Vite 创建一个 Vue3 工程。
...
可以看到工程依赖了 pinia2、vue3 和 vue-router4。pinia2 和 vue-router4 以后再单独学习。

开发依赖主要是 vite、eslint、prettier 和 vitest。
...
这个文件夹下的内容会被原样拷贝到生成的 `./dist` 目录，适合放静态文件，以及传统 HTML 里通过 `<script>` 标签引入的 js。

### src 文件夹

src 放 Vue 工程的源码，包括组件、图片资源，以及需要打包到一起的 js 和 css。
...
开发 Vue 项目时需要一个本地服务来预览部署后的效果，做到所见即所得。
