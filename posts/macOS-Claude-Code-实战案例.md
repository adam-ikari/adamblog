...
前面六篇讲了怎么准备环境、怎么安装配置、怎么选型、字段怎么写、报错怎么排查。这篇把所有知识点串起来——**从零用国产模型接入 Claude Code，端到端做一个真实的小项目**，再复盘过程中的费用和踩坑。

我们拿 DeepSeek 当本次实战的提供商（性价比高、国内直连、代码能力强，理由见 [选型指南](/posts/macOS-国产模型提供商选型指南)），做一个**命令行待办工具 `todo`**：支持增删查改，数据持久化到本地 JSON 文件。
...
没装好的话，回 [环境准备教程](/posts/macOS-环境准备教程) 和 [安装配置教程](/posts/macOS-安装和配置-Claude-Code-教程) 补齐。
...
编辑 `~/.claude/settings.json`（写法见 [配置详解](/posts/macOS-Claude-Code-配置详解)）：
...
`/init` 会自动分析目录、生成 `CLAUDE.md`，再手动补点规范进去：
...
模型会创建文件、写代码、配 `package.json`。你可以让它边写边解释，也可以让它一把写完再 review。
...
它去读代码、定位 bug、改掉。这就是 Claude Code 理解项目上下文的好处——它记得自己刚写的代码。

## 第八步：费用复盘

用 `/cost` 看本次会话的 token 用量和费用。DeepSeek 按量计费，做这么个小工具，全程对话加代码生成，花费通常在**几毛钱以内**，甚至几分钱。
...
这一个实战把系列前六篇的知识全用上了：环境准备让 Node.js 就绪，安装配置把 Claude Code 装好，选型定了 DeepSeek，配置详解告诉我们用 `AUTH_TOKEN` + `env` 块 + 档位映射，报错排查教我们先 curl 验证、遇问题按手册查，cc-Switch 则用来管多模型。

最后用几分到几毛钱的成本，端到端做出了一个能用的 CLI 工具。这就是国产模型 + Claude Code 在国内落地的真实体感：**够用、便宜、可控**。

整个 macOS 配置国产大模型 API 系列到这里就结束了。希望这套从环境到实战的完整教程，能帮你在国内顺顺当当用上 AI 辅助编程。
