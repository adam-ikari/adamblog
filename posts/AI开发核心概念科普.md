...
用户提问
    ↓
┌─────────────┐
│  检索阶段   │  ← 把问题转成向量，去知识库找最相关的片段
│   (Search)  │     如：向量数据库、搜索引擎、文档索引
└─────────────┘
    ↓
┌─────────────┐
│  增强阶段   │  ← 把检索到的片段和原始问题拼接成 prompt
│  (Augment)  │     "根据以下资料回答问题：[资料片段]"
└─────────────┘
    ↓
┌─────────────┐
│  生成阶段   │  ← 模型基于增强后的 prompt 生成回答
│  (Generate) │     回答更精准、可溯源、不易幻觉
└─────────────┘
```
...
├── 描述："搜索互联网获取最新信息"
├── 工具：
│   ├── search_web(query) → 调用搜索引擎
│   └── parse_results(html) → 提取关键信息
├── Prompt 模板：
│   └── "根据搜索结果回答问题：{results}"
└── 执行逻辑：
    ├── 步骤 1：调用搜索引擎
