...
> 🤖 **AI 辅助开发 Tip**
>
> **传统方式**：阅读 Android Automotive OS 官方架构文档，手动绘制架构图理解各层关系。面对 HAL、Framework、Native Services 等概念时，容易混淆它们的职责边界。需要反复查阅多份文档才能建立完整的认知框架。
>
...
> 🤖 **AI 辅助开发 Tip**
>
> **传统方式**：阅读车载 UI 设计指南，手动将 "大字体、大按钮、高对比度" 等原则应用到每个 Composable 函数中。需要反复调整 `fontSize`、`padding`、`Modifier` 参数，通过肉眼比对确保所有组件风格一致。驾驶模式检测逻辑容易遗漏边界情况（如速度为 0 时的状态切换）。
>
...
> 🤖 **AI 辅助开发 Tip**
>
> **传统方式**：阅读 Car API 官方文档，手动编写获取车辆属性的代码。Car API 的初始化流程较复杂（创建 `Car` 对象 → 获取 `CarSensorManager` → 注册监听器 → 处理回调 → 断开连接），容易遗漏资源释放步骤。不同属性的获取方式差异大（`getFloatProperty` vs `registerListener`），需要反复查阅文档确认参数含义。
>
