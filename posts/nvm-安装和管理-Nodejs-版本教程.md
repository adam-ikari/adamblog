...
日常开发里，不同项目依赖的 Node.js 版本经常对不上。旧项目可能只有 Node.js 14 才跑得动，新项目又要求 20 甚至更高。全局只留一个版本，切来切去早晚出问题：旧项目报语法错误、依赖不兼容，升级 Node.js 后全局工具集体失效，团队里版本一不一致就是"我这边没问题"那一套。

手动卸了装、装了卸既繁琐又容易出错。版本管理工具就是冲这个来的——一台机器上装多个版本，随时切。

![nvm 工作流：从安装到项目级版本管理](/posts/nvm-安装和管理-Nodejs-版本教程/nvm-workflow.svg)
...
nvm 是管理 Node.js 版本最成熟的工具，几条命令就能搞定多版本安装和切换。日常大部分场景，记住下面这几个就够了：
...
再配上 `.nvmrc` 做项目级版本管理，把版本号随代码一起进版本控制，团队里就不再有"我这边 Node 版本不对"的扯皮。
