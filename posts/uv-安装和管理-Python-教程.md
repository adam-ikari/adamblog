...
Python 开发者对环境管理的折腾都不陌生，几乎是从入门第一天就开始的。项目 A 要 3.10，项目 B 要 3.12，手动装来切去费时费力；`python -m venv` 建环境、`source` 激活、再 `pip` 装依赖，步骤一多就容易忘；`pip install` 解析大型项目依赖动辄几分钟，CI 的时间大半耗在装包上；更烦的是工具链割裂——版本管理靠 pyenv，虚拟环境靠 venv，包管理靠 pip，项目管理靠 poetry，几个工具各管一段，彼此还不兼容。

受够了这些的话，**uv** 大概就是你要找的答案。

![uv 一个工具替掉一整套 Python 工具链](/posts/uv-安装和管理-Python-教程/uv-replaces-toolchain.svg)
...
最直观的就是快——Rust 写的，加上全局包缓存，装依赖比 pip 快 10 到 100 倍。功能也全，一个工具就把 Python 安装、版本管理、虚拟环境、包管理、项目管理、工具安装都包了。兼容性方面不挑食，pyproject.toml 标准和 requirements.txt 都支持，迁移成本低。再加上严格的依赖解析和锁文件，环境可复现这点也有保障。
...
uv 还提供了完整的项目管理功能，定位类似 poetry，但速度快得多。
...
uv 一个工具就把 Python 开发里那套东拼西凑的工具链替了，从装 Python 到管项目一条龙。日常大部分场景，下面这些命令够用：
...
还在 pip + venv + pyenv + poetry 凑一套的，可以试试 uv，Rust 带来的速度差确实明显。
