...
在前面的教程里，我们已经知道了 Claude Code 可以通过 `ANTHROPIC_BASE_URL` + `ANTHROPIC_AUTH_TOKEN` 接入第三方 API 提供商。但面对市面上众多的国产大模型 API，该选哪一个？

这篇文章不教你「怎么配」（那是 [安装配置教程](/posts/macOS-安装和配置-Claude-Code-教程) 的事），而是帮你回答「配哪个」。我会从**认证方式、价格、模型能力、适用场景**四个维度横向对比主流国产提供商，最后给出一个选型决策树。

> ⚠️ 大模型 API 的价格、模型版本、接入地址会频繁变动。本文基于撰写时（2026 年 6 月）的公开信息整理，**具体请以各提供商官网最新公示为准**。

## 选型前要搞清楚的三件事

在比较提供商之前，先想清楚自己的情况，这会直接决定你该往哪个方向选。
...
讯飞推出的编程专用套餐，最大的特点是**一个套餐内可以切换多种模型**（星火、GLM、DeepSeek 等），相当于用订阅价拿到多个模型的访问权。适合不想在多个平台分别充值、希望一个账号搞定多种模型切换的用户。
...
DeepSeek 是国产编程大模型里口碑最好的一家之一，模型本身的代码能力经过大量开发者验证，价格也以「便宜」著称。按量计费，用多少花多少，非常适合预算敏感的轻度到中度用户。
...
硅基流动是模型聚合平台，接入了大量开源模型（DeepSeek、Qwen、GLM 等）。新用户常有免费额度赠送，是国内开发者「白嫖」体验多种模型的首选。
...
清华系团队，GLM 系列模型在中文理解和代码能力上都比较均衡，国内访问稳定，合规性强。
...
阿里云背书，Qwen 系列模型能力全面，DashScope 平台稳定可靠，企业用户首选之一。
...
以超长上下文窗口见长，适合需要让模型一次性读取大量代码、长文档的场景。
...
海外模型聚合平台，能在一个 API 下访问 Claude、GPT、Gemini 以及各种开源模型。**需要代理访问**。
...
选定提供商后，**不要急着在 Claude Code 里长跑**，先用最小成本验证：
...
跑顺了再考虑用 [cc-Switch](/posts/macOS-配置cc-Switch教程) 把多个提供商管理起来，方便额度耗尽时切换。

## 选完之后：用 cc-Switch 管理多个国产模型

选型最难的不是挑出「最好」的那家，而是现实里你往往会**同时用好几家**——DeepSeek 当主力、讯飞 Coding Plan 兜底、硅基流动薅免费额度。手动改 settings.json 在它们之间切来切去既累又容易写错（最典型的就是忘了把 `ANTHROPIC_API_KEY` 换成 `ANTHROPIC_AUTH_TOKEN`，结果 401）。

这就是 [cc-Switch](/posts/macOS-配置cc-Switch教程) 的用武之地：把每家国产模型存成一个配置，一条命令切换。
...
如果还想连权限、MCP、档位映射整套配置一起切换，用 `@aravhawk/cc-switch` 的 Profile 模式，为每家国产模型建一份完整 Profile。
