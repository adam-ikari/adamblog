...
![Lisp 漫画](/posts/【文档翻译】Lisp 的根源/Lisp的根源.jpg)
...
## 从 McCarthy 记号到 Common Lisp

McCarthy 的论文使用的是数学记号，而不是 S-表达式。他的记号翻译为代码的对应关系如下：
...
McCarthy 用 `label` 来实现递归定义。在数学上，这等价于不动点组合子（fixed-point combinator），即 Y 组合子。Y 组合子的意义在于：即使一门语言没有原生的递归支持，只要它支持高阶函数，就可以通过 Y 组合子实现递归。
...
这意味着：`Y f = f(Y f)`，即 Y 作用于 f 的结果就是 f 的不动点。

## 自引用：Lisp 的核心突破

现在我们来到了全文最核心的问题：**Lisp 如何做到"用自身来编写自身"？**

这看起来是一个悖论。当我们写 eval 的时候，eval 本身就是一段 Lisp 代码，而 eval 的功能就是求值 Lisp 代码——包括求值 eval 自身。这就像一个包含自身的集合，或者一本描述所有书的书。

在 McCarthy 之前，没有人知道如何在不依赖已有解释器的情况下定义一门语言的自解释器。McCarthy 的突破在于：
...
让我们用一个具体的例子来理解。假设我们要用 eval 来求值表达式 `(car '(a b c))`：
...
注意外层的 quote：我们用 `'` 保护了整个表达式，使其作为数据传入 eval，而不是被直接求值。eval 接收到这个列表后，识别出操作符是 `car`，对参数 `'(a b c)` 求值得到 `(a b c)`，然后应用 `car` 得到 `a`。

这就是 Lisp 的魔力：**程序可以把自己当作数据来操作**。这种能力被称为**同像性**（homoiconicity），源自希腊语 homo（相同）和 icon（形象），意思是"代码和数据的形象相同"。

## 为什么这很重要

你可能会问：一个语言能用自身编写，这到底有什么实际意义？

答案是：**这是编程语言表达能力的终极检验**。如果一门语言不能方便地操作自身的结构，那么每当程序员需要处理"代码"时，就不得不退回到字符串操作——就像在 C 中用 `sprintf` 拼接 SQL 语句一样原始而危险。

Lisp 的宏系统就是建立在这种能力之上的。Lisp 的宏不是简单的文本替换（像 C 的 `#define`），而是**对代码结构的变换**——因为代码本身就是数据结构。一个 Lisp 宏接收代码作为列表，对其进行任意变换，然后返回新的代码。这赋予了 Lisp 程序员一种其他语言无法企及的力量：**定义新的语言构造**。

例如，如果你觉得 Lisp 缺少一个 `while` 循环，你不需要等待语言设计者添加它——你可以自己写一个宏：
...
这就是为什么 Graham 说"Lisp 之所有与众不同，是因为它可以用自身来编写"。这不仅仅是一个理论上的优雅性质，而是赋予了程序员**改变语言本身**的能力。
...
- **Python** 加入了垃圾回收、高阶函数、动态类型
- **JavaScript** 加入了闭包、一等函数、eval
- **Ruby** 加入了闭包、动态类型、宏（通过元编程）
- **Rust** 的宏系统借鉴了 Lisp 的卫生宏概念
- **Julia** 直接采用了 S-表达式作为 AST 表示

Graham 的观察是：**编程语言的进化方向，就是逐渐变成 Lisp**。如果你把一门现代语言的所有"创新"特性剥开来看，你会发现它们几乎都来自 Lisp——只是换了一种语法。
